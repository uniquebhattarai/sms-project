...
import { 
  Sparkles, Target, AlertCircle, Calendar, CheckCircle, Award, Users 
} from "lucide-react";
...
export default function AdminPredictionDashboard() {
  ...
  useEffect(() => {
    const fetchClasses = async () => {
      ...
      const res = await apiConnector("GET", "/class_list/", null, { Authorization: `Bearer ${token}` });
      setClasses(res.data);
    };
    fetchClasses();
  }, []);
  ...
  const fetchStudents = async (classId) => {
    ...
    setSelectedStudent("");
    const token = localStorage.getItem("access");
    const res = await apiConnector("GET", `/get_student_by_class/${classId}`, null, { Authorization: `Bearer ${token}` });
  ...
  const fetchPrediction = async () => {
    ...
    try {
      ...
      // Fetch prediction
      const modelData = await apiConnector("GET", `/mark_prediction_by_id/${selectedStudent}`, null, { Authorization: `Bearer ${token}` });
      const score = await apiConnector("POST", "/score_prediction/", modelData.data, { Authorization: `Bearer ${token}` });
      ...
      const dashboardRes = await apiConnector("GET", `/student_dashboard/${selectedStudent}`, null, { Authorization: `Bearer ${token}` });
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="mb-10 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-600 font-medium">Select a student to view predicted outcome & metrics</p>
          </div>
        </div>
        ...
        <div className="mb-10 grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="flex flex-col">
            <label className="font-medium text-gray-700 mb-2">Select Class</label>
            <select
              className="px-4 py-3 rounded-xl border border-gray-300 bg-white shadow-sm text-gray-900 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
              onChange={(e) => {
            ...
            >
              ...
              {classes.map((c) => (
                <option key={c.id} value={c.id}>Class {c.level}</option>
              ))}
            </select>
          </div>

          <div className="flex flex-col">
            <label className="font-medium text-gray-700 mb-2">Select Student</label>
            <select
              className="px-4 py-3 rounded-xl border border-gray-300 bg-white shadow-sm text-gray-900 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
          ...
          <div className="flex items-end">
            <button
              ...
              className="w-full px-6 py-3 rounded-xl bg-indigo-600 disabled:bg-gray-400 text-white font-semibold shadow-md hover:shadow-xl hover:scale-[1.03] transition-all duration-200"
            >
              🔮 Predict Score
            </button>
          </div>
            ...
            <div className="bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 rounded-3xl shadow-2xl p-10 text-white relative overflow-hidden">
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <h2 className="text-3xl font-bold">{studentName || "Student"}'s Score Prediction</h2>
                ...
                <div className="grid md:grid-cols-3 gap-8 items-end">
                  <div className="md:col-span-2">
                    <div className="flex items-end gap-6">
                      <div>
                        <p className="text-8xl font-black tracking-tight">{scaledPrediction?.toFixed(1)}</p>
                      ...
                      <div className="bg-white bg-opacity-10 backdrop-blur-md rounded-2xl px-8 py-5 mb-4 border border-white border-opacity-20">
                        ...
                        <p className="text-5xl font-black text-purple-600">{getScoreGrade(scaledPrediction)}</p>
                      </div>
                    </div>
                  ...
                  <div className="bg-white bg-opacity-10 backdrop-blur-md rounded-2xl p-6 border border-white border-opacity-20">
                    ...
                    <p className="text-sm text-gray-800 leading-relaxed">
                      The system evaluates attendance, assignments, internal marks, and past performance to estimate the final exam score.
                    </p>
                  </div>
                </div>
              </div>
