...
const Card = ({ children, className, hover = false, gradient = false }) => (
  <div className={`bg-white rounded-3xl shadow-xl border border-gray-100 transition-all duration-500 ease-out ${hover ? 'hover:shadow-2xl hover:-translate-y-2 hover:scale-[1.02]' : ''} ${gradient ? 'bg-gradient-to-br from-white to-gray-50' : ''} ${className || ""}`}>
    {children}
  </div>
);
...
function AdminDashboard({ role = "admin" }) {
  ...
  const [error, setError] = useState(null);
    ...
    const fetchDashboard = async () => {
      ...
      // Get token from localStorage - the app stores it as "access"
        ...
        <Card className="max-w-md mx-auto text-center">
          <CardContent>
            ...
            <div className="bg-gray-100 rounded-lg p-4 mb-6 text-left text-sm">
              ...
              <p><strong>Access token exists:</strong> {localStorage.getItem("access") ? "Yes" : "No"}</p>
              <p><strong>Access token length:</strong> {localStorage.getItem("access")?.length || 0}</p>
              <p><strong>Access token preview:</strong> {localStorage.getItem("access")?.substring(0, 20)}...</p>
              <p><strong>Role:</strong> {localStorage.getItem("role") || "Not set"}</p>
              <p><strong>Refresh token exists:</strong> {localStorage.getItem("refresh") ? "Yes" : "No"}</p>
              <p><strong>API Base URL:</strong> https://prejjj.pythonanywhere.com/api</p>
              <p><strong>Dashboard Endpoint:</strong> /dashboard/</p>
              <p><strong>Full URL:</strong> https://prejjj.pythonanywhere.com/api/dashboard/</p>
            </div>
              ...
              <button
                onClick={async () => {
                  try {
                    ...
                    const response = await fetch("https://prejjj.pythonanywhere.com/api/dashboard/", {
                      method: "GET",
                      headers: {
                        "Authorization": `Bearer ${token}`,
              ...
              <div className="flex items-center justify-between mb-8">
                <div>
                  <h2 className="text-3xl font-bold text-gray-800 mb-2">
                    {role === "admin" ? "School Attendance Trends" : "My Class Attendance"}
                  </h2>
          ...
          <Card hover gradient>
            <CardContent>
              <div className="flex items-center justify-between mb-8">
                <div>
                  <h2 className="text-3xl font-bold text-gray-800 mb-2">
                    {role === "admin" ? "Class Performance" : "My Class Performance"}
            </h2>
                  <p className="text-gray-600">
                    {role === "admin" 
        ...
        <Card hover gradient>
          <CardContent>
            <div className="flex items-center justify-between mb-8">
              <div>
                <h2 className="text-3xl font-bold text-gray-800 mb-2">
                  {role === "admin" ? "Recent Activities" : "My Recent Activities"}
                </h2>
                <p className="text-gray-600">
                  {role === "admin" 
              ...
              {dashboard.recent_activities && dashboard.recent_activities.length > 0 ? (
                    ...
                    <div className="flex-1 min-w-0 relative z-10">
                      ...
                      <div className="flex items-center gap-6 text-sm text-gray-500">
                        <span className="flex items-center gap-2 bg-gray-100 px-3 py-2 rounded-full">
                          <FiCalendar className="w-4 h-4" />
                          <span className="font-medium">{act.date ? new Date(act.date).toLocaleDateString() : 'Unknown date'}</span>
                        </span>
                        <span className="flex items-center gap-2 bg-gray-100 px-3 py-2 rounded-full">
                          <FiLayers className="w-4 h-4" />
                          <span className="font-medium">{act.class_name || 'Unknown class'}</span>
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="text-center py-16">
                  ...
                  <h3 className="text-xl font-semibold text-gray-600 mb-2">
                    {role === "admin" ? "No recent activities" : "No recent teaching activities"}
                  </h3>
...
function SummaryCard({ icon, label, value, color = "from-blue-500 to-blue-600", trend, delay = 0 }) {
  return (
    <div 
    ...
    >
      ...
      <div className={`relative w-20 h-20 bg-gradient-to-br ${color} rounded-3xl flex items-center justify-center mb-6 group-hover:scale-110 group-hover:rotate-3 transition-all duration-500 shadow-lg`}>
        <div className="text-3xl text-white drop-shadow-sm">{icon}</div>
        {/* Subtle glow effect */}
        <div className={`absolute inset-0 bg-gradient-to-br ${color} rounded-3xl opacity-0 group-hover:opacity-20 blur-xl transition-opacity duration-500`}></div>
      </div>
      ...
      <div className="text-gray-600 font-semibold mb-4 text-center relative z-10">{label}</div>
